...
class RoleView(nextcord.ui.View):
    ...
    @nextcord.ui.button(label = "Equipe rouge", emoji = ":RED:", style = nextcord.ButtonStyle.red, custom_id = custom_id(VIEW_NAME, config.TEAM_RED_ID))
    async def red_button(self, button, interaction):
        await self.handle_click(button, interaction)

    @nextcord.ui.button(label = "Equipe bleue", emoji = ":BLUE:", style = nextcord.ButtonStyle.primary, custom_id = custom_id(VIEW_NAME, config.TEAM_BLUE_ID))
